...
export default function MetaMaskDialog() {
    ...
    const list = [
      {
        ...
        content: 'Trusted by over 30 million users to buy, store, send and swap crypto securely'
      },
      {
        icon: imagesMetamask?.wallet,
        content: 'The leading crypto wallet & gateway to blockchain apps built on Ethereum Mainnet, Polygon, Optimism and many other networks'
      },
      {
        icon: imagesMetamask?.lock,
        content: 'Puts you in control of your digital interactions by making power of cryptography more accessible'
      },
    ] 
    ...
    const handleInstall = () => {
      ...
      if(browserName?.includes("Edge")) {
        ...
        window.open("https://addons.opera.com/en-gb/extensions/details/metamask-10/", "_blank")
      }
    }

    return (
      <Dialog open={isMetamaskDialog} PaperProps={{
        style: {
          width: width < 576 ? "100%" : '50%'
        }
      }} fullScreen={width && width < 576} onClose={() => dispatch(toggleMetaMaskDialog())}>
        <Box component={"div"} sx={{
          ...
          <Box component={"div"} className='d-flex justify-content-center mt-5'>
            <Box component={"img"} src={imagesMetamask?.logo} width={40} height={35} alt="..."/>
            <Box component={"h2"} className='text-white ms-1'>
              METAMASK
            </Box>
          </Box>
          <Box component={"div"} className='position-absolute' sx={{ top: 10, right: 10 }}>
            <CloseIcon onClick={() => dispatch(toggleMetaMaskDialog())} sx={{ color: '#8a77bc' }}/>
          </Box>
          <Box component={"div"} className='p-3 ps-5 pe-5 d-flex justify-content-center'>
            {menus && menus?.map((mn, i_mn) => (
              <Box key={i_mn} component={"div"} onClick={() => {
                setMenuK(i_mn)
              }} className='p-1 ps-2 pe-2' sx={{
          ...
          {menuK === 0 ? (
            <Box>
              <Box component={"div"} className='p-2 ps-4 pe-4'>
                {list && list?.map((item, i_item) => (
                  <Box key={i_item} component={"div"} className='rounded mb-2 d-flex align-items-center' sx={{
                    border: '0px solid #857cab',
                    borderWidth: "0.1px"
                  }}>
                    <Box className="p-2 ps-3 pe-3" component={"div"}>
                      <Box component={"img"} src={item?.icon} alt="..." width={25} height={25}/>
                    </Box>
                    <Box component={"div"} className='p-2 pt-3 pb-3' sx={{
                      color: '#afa5d9',
                      fontSize: '14px'
                    }}>{item?.content}</Box>
                  </Box>
              ...
              <Box component={"div"} className="mt-3 mb-5 d-flex justify-content-center">
                  <Box component={"div"} className="d-flex rounded p-2 justify-content-center align-items-center" sx={{ 
                    width: '80%', 
                    background: "linear-gradient(180deg, #843ff0, #7748ed)",
                  }}>
                    <Box component={"img"} src={imagesMetamask?.arrowD} alt="..." width={15} height={15}/>
                    <Box component={"div"} className='text-white ms-2 cursor-pointer' onClick={handleInstall}>
            ...
            <Box component={"div"}>
              ...
              <Box component={"div"} className='mt-3 mb-5 pt-1'>
                <Box className="mt-2" component={"div"}>
                    ...
                    <Typography className='mt-2' sx={{ color: '#fff', fontSize: '14px' }}>MetaMask Mobile App</Typography>
                </Box>
              </Box>
            </Box>
          )}
