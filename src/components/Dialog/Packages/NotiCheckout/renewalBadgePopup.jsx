...
export default function RenewalBadgePopup(props) {
    ...
    const {isCheckRenewalPopup} = useSelector((state) => state.walletReducer);
    const handleClose = () => {
    ...
    const handleConfrim = () => {
        const cancelPack = JSON.parse(localStorage.getItem("cancelPackage"));
        dispatch(
            deleteCurrentPackage({
                packageId: +cancelPack?.id,
                paypalSubsId: cancelPack?.packagePaypalId
            })
        );
        dispatch(closeRenewalBadgePopup());
    };
