...
import {clearCurrentPromotionShare} from "../../../redux-saga-middleware/reducers/promotionReducer";

export default function ShareTour() {
    const {width, height} = useWindowDimensions();
    const {isShare} = useSelector((state) => state.authReducer);
    const {user} = useSelector((state) => state.userReducer);
    const {currentPromotionShare} = useSelector(state => state.promotionReducer)
    ...
    const handleClose = () => {
        ...
        dispatch(clearCurrentPromotionShare());
    };
                                ...
                                onClick={
                                    () => {
                                        ReactGA.event("share", {
                                            ...
                                            transport: "xhr",
                                            shareType: "twitter",
                                            tournamentId: currentPromotionShare?.id,
                                            tournamentName: currentPromotionShare?.tournamentName,
                                            gameId: currentPromotionShare?.tournamentInfors?.game?.id,
                                            gameName: currentPromotionShare?.tournamentInfors?.game?.gameName,
                                        });
                                    }
                                }
                            >
