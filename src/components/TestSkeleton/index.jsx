...
class TestSkeleton extends Component {
  ...
  componentDidMount() {
    ...
    fetch(
      "http://192.168.1.144:3009/api/promotions/list-promotion?typeTournament=daily"
    )
      ?.then((res) => {
        return res?.json();
      })
  ...
  render() {
    ...
    return (
      <div>
        ...
        {isNodata && !check && (
          <Box className="text-white">This is no-data</Box>
        )}
        {!isNodata && !check && list && list?.length > 0 && (
          <Box className="text-white">This is data</Box>
        )}
      </div>
    );
  }
