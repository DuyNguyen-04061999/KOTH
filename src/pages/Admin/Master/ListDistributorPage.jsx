...
export default function ListDistributorPage() {
    ...
    return (
        <Box component={"div"} className='bg-white text-dark' sx={{
            height
        }}>
            {roles && roles?.length > 0 && roles?.includes("master") ? (
                <Box component={"div"} className=' d-flex p-2'>
                    {listDistributor && listDistributor?.length > 0 && listDistributor?.map((dis, i_dis) => (
                        <Box component={"div"} className='card me-2 p-2 pt-3 pb-3 cursor-pointer d-flex flex-row justify-content-between' key={i_dis}>   
                            {dis?.userName}
