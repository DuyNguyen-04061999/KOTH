...
export default function Tournament() {
  ...
  const renderS = () => {
    return (
      <Box className="m-2">
        <Box className="p-2 bg-info text-white card mb-2" sx={{
            width: 'fit-content'
          }} onClick={() => {
        ...
        <Box className="d-flex">
          {tournaments && tournaments?.length > 0 && tournaments?.map((tournament, i_) => (
            <Box key={i_} className="card bg-white text-dark p-2 mb-2 me-2" sx={{
              width: 'fit-content'
            }}>
              {tournament?.tournamentName}
              <Box className="p-1 bg-info text-white card mt-2" sx={{
                width: 'fit-content'
              }}>
                JOIN
              </Box>
