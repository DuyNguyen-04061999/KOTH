...
export default function SystemBalance() {
  ...
  const fetchDataPromotion = async (search) => {
    try {
      ...
      const response = await fetch(
        search
          ? `${process.env.REACT_APP_PROMOTION_URL}/api/system/balance/list-end-promotion?search=${search}`
  ...
  const fetchDataUser = async (search) => {
    try {
      ...
      const response = await fetch(
        search
          ? `${process.env.REACT_APP_PROMOTION_URL}/api/system/balance/list-user?search=${search}`
  ...
  const fetchDataUserVerify = async (search) => {
    try {
      ...
      const response = await fetch(
        search
          ? `${process.env.REACT_APP_PROMOTION_URL}/api/system/balance/list-user-verify?search=${search}`
  ...
  const handleGetExcel = async (type) => {
    try {
      ...
      const response = await fetch(
        type
          ? `${process.env.REACT_APP_PROMOTION_URL}/api/system/balance/export-promotion-result-excel?type=1`
  ...
  const handleUpdateScorePromotion = async () => {
    if (!selectedPromotionId || !selectedUserId || !score || !datePlay) {
      ...
      await fetch(
        `${process.env.REACT_APP_PROMOTION_URL}/api/system/balance/update-score-promotion`,
        {
          method: "POST",
          body: JSON.stringify({
  ...
  const handleUpdateScorePromotionVerify = async () => {
    if (!selectedUserVerifyId) {
      ...
      await fetch(
        `${process.env.REACT_APP_PROMOTION_URL}/api/system/balance/update-phone-verify`,
        {
          method: "POST",
          body: JSON.stringify({
        ...
        >
          <Box>
            ...
            <Autocomplete
              ...
              getOptionLabel={(option) =>
                option?.tournamentName || selectedPromotion || ""
              }
              sx={{ width: 400 }}
            ...
            <Autocomplete
              ...
              options={listUser}
              getOptionLabel={(option) =>
                option?.userNickName || selectedUser || ""
              }
              sx={{ width: 400 }}
              value={selectedUser || ""}
            ...
            <TextField
              placeholder="Date"
              onChange={(e) => setDatePlay(e.target.value)}
              type="date"
              className="mt-2 ms-1"
            />

            <Button
              className="bg-info text-white p-2 p-3 ms-2 mt-2"
              onClick={handleUpdateScorePromotion}
            >
              Process
            </Button>
          </Box>
          <Box component={"div"} className="mt-3">
            ...
            <Autocomplete
              ...
              getOptionLabel={(option) =>
                option?.userNickName || selectedUserVerify || ""
              }
              sx={{ width: 400 }}
            ...
            />
            <Button
              className="bg-info text-white p-2 p-3 mt-2"
              onClick={handleUpdateScorePromotionVerify}
            >
              Process
            </Button>
          </Box>
          ...
          <Box component={"div"} className="mt-3">
            ...
            <Box>
              <CSVLink
                data={csvData || []}
                filename={dateType ? "all_results" : "today_results"}
              >
                <Button className="bg-success text-white p-2 p-3 mt-2">
                  Process Excel
                </Button>
              </CSVLink>
            </Box>
